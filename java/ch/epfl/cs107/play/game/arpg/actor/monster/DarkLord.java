...
public class DarkLord extends Monster {
    ...
    private DarkLord(Area area, Orientation orientation, DiscreteCoordinates position, float currentHP) {
        ...
        spellWait =
                MIN_SPELL_WAIT_DURATION + RandomGenerator.getInstance().nextFloat() * (MAX_SPELL_WAIT_DURATION - MIN_SPELL_WAIT_DURATION);
    ...
    /**
     * tests and selects a random orientation of all four possible if it can allow a fireSpell to enter the area in the cell following
     * DarkLord in that orientation, then orientates DarkLord accordingly if a fireSpell can be placed
     *
    ...
    private boolean strategicOrienting() {
        ...
        do {
            ...
            firePositionTrial = Collections.singletonList(getCurrentMainCellCoordinates().jump(randomOrientation.toVector()));
    ...
    private DarkLord teleport() {
        int currentX = getCurrentMainCellCoordinates().x, currentY = getCurrentMainCellCoordinates().y;
    ...
    public void update(float deltaTime) {
        if (!getIsDead()) {
            ...
            if (spellWait <= 0) {
                randomState();
                spellWait =
                        MIN_SPELL_WAIT_DURATION + RandomGenerator.getInstance().nextInt(MAX_SPELL_WAIT_DURATION - MIN_SPELL_WAIT_DURATION);
            }
