...
public class Dialog implements Graphics {
    ...
    public Dialog(String text, String backgroundName, Area area) {
        ...
        final Vector fourthLineAnchor = new Vector(0.5f, height - 5.5f * FONT_SIZE);

        sprite = new ImageGraphics(ResourcePath.getSprite(backgroundName), width, height, null, Vector.ZERO, 1.0f, 3000);

        lines = new TextGraphics[4];

        lines[0] = new TextGraphics("", FONT_SIZE, Color.BLACK, null, 0.0f, false, false,
        		firstLineAnchor, TextAlign.Horizontal.LEFT, TextAlign.Vertical.TOP,  1.0f, 3001);
        
        lines[1] = new TextGraphics("", FONT_SIZE, Color.BLACK, null, 0.0f, false, false,
        		secondLineAnchor, TextAlign.Horizontal.LEFT, TextAlign.Vertical.TOP, 1.0f, 3001);
        
        lines[2] = new TextGraphics("", FONT_SIZE, Color.BLACK, null, 0.0f, false, false,
        		thirdLineAnchor, TextAlign.Horizontal.LEFT, TextAlign.Vertical.TOP, 1.0f, 3001);

        lines[3] = new TextGraphics("", FONT_SIZE, Color.BLACK, null, 0.0f, false, false,
        		fourthLineAnchor, TextAlign.Horizontal.LEFT, TextAlign.Vertical.TOP, 1.0f, 3001);
    ...
    public void draw(Canvas canvas) {
        ...
        lines[0].setAnchor(new Vector(0.3f, height/4.005f - FONT_SIZE));
        lines[1].setAnchor(new Vector(0.4f, height/4 - 2.5f * FONT_SIZE));
        lines[2].setAnchor(new Vector(0.4f, height/4 - 4 * FONT_SIZE));
        lines[3].setAnchor(new Vector(0.4f, height/4 - 5.4f * FONT_SIZE));
        for(int i = 0; i < 4; i++){
